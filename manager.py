...
class APIManager:
    ...
    def _default_api_policy(self) -> dict:
        return {
            "timeout_seconds": int(self.config.get("timeout", 30)) if hasattr(self, "config") else 30,
    ...
    def _normalize_config(self, data: dict) -> dict:
        ...
        base["timeout"] = int(base.get("timeout", 30) or 30)
    ...
    def set_listen(self, host: str, port: int, timeout: int):
        with self.lock:
    ...
    def detect_models_for_api(self, api: dict) -> list:
        ...
        timeout_seconds = int((api.get("policy") or {}).get("timeout_seconds", self.config.get("timeout", 30)) or 30)
        target = self._join_url(api["url"], "models")
        headers = dict(api.get("headers") or {})
        self.log(f"-> 检测模型 [{api['name']}] GET {target}")
        resp = requests.get(target, headers=headers, timeout=timeout_seconds, allow_redirects=False, proxies=proxies)
    ...
    def _rewrite_model_for_api(self, api: dict, body: bytes) -> tuple[bytes, str, Optional[str]]:
        """
        返回: (转发body, 实际发送模型名, 错误信息)
        严格单绑定规则：
        - 每个上游只允许使用它配置页里选中的 selected_model
        - 请求 default_alias 时，自动改写为 selected_model
        - 请求其他模型名时，只有与 selected_model 完全一致才允许转发
        ...
        if requested_model != selected:
            return body, requested_model, f"当前上游仅允许模型 {selected}，不接受 {requested_model}"
    ...
    def forward(self, subpath: str, method: str, headers: dict, params: dict, body: bytes):
        ...
        while current_idx is not None and current_idx not in visited and 0 <= current_idx < len(enabled_apis):
            ...
            api = enabled_apis[current_idx]
            policy = api.get("policy") or {}
            timeout_seconds = int(policy.get("timeout_seconds", self.config.get("timeout", 30)) or 30)
            ...
            send_body, actual_model, model_err = self._rewrite_model_for_api(api, body)
            if model_err:
                last_err = model_err
                self.log(f"   x [{api['name']}] {model_err}")
                next_idx = self._decide_next_index(enabled_apis, current_idx, visited)
                if next_idx is None:
                    self.log(f"!! [{api['name']}] 无可继续切换的目标")
                    break
                action_name = "指定目标" if str((api.get("policy") or {}).get("action", "next")) == "specific" else "下一个"
                self.log(f"   -> [{api['name']}] 模型不匹配，切换到 {action_name}: [{enabled_apis[next_idx]['name']}]")
            ...
            for attempt in range(retries + 1):
                ...
                self.log(f"-> 尝试 [{api['name']}] {round_text} {method} {target}{model_text}")
                try:
                    ...
                    self.log(f"   x [{api['name']}] {last_err}")
                    if attempt < retries and policy.get("trigger_timeout", True):
                        self.log(f"   ↻ [{api['name']}] 满足超时重试条件，准备重试")
                        continue
                    break
                ...
                if failure is None:
                    self.log(f"   ✓ [{api['name']}] 成功 ({resp.status_code})")
                ...
                if should_retry:
                    self.log(f"   ↻ [{api['name']}] 满足重试条件，准备重试")
                    continue
                break
            ...
            if next_idx is None:
                ...
                break
            action_name = "指定目标" if str((api.get("policy") or {}).get("action", "next")) == "specific" else "下一个"
            self.log(f"   -> [{api['name']}] 失败后切换到 {action_name}: [{enabled_apis[next_idx]['name']}]")
            current_idx = next_idx

        self.log(f"!! 所有上游均失败：{last_err}")
